...
**Mentor-directed "Phase 0.5" is in progress**: before full-fledged feature development, prove core infra works in isolation — cron job triggering, Service Bus messaging across services, CRUD on DB, CRUD on Redis. See the Progress checklist below for what's done. Once Phase 0.5 is complete, the plan shifts to real domain/business logic across all services.
...
- A Redis CRUD proof (`Endpoints/CacheEndpoints.cs`, `IConnectionMultiplexer` via `AddRedisClient("cache")`) was drafted here as Identity.Api is the only working service — deliberately not finished/tested; paused to prioritize Phase 0.5's other items and real feature work. Revisit when Redis is actually needed by a real feature (most likely Booking.Api for availability caching, matching common patterns for this kind of domain) or when returning to close out the Phase 0.5 checklist.
...
- No availability-cache (Redis) — every slot-picker interaction hits Postgres fresh via Booking.Api; a natural next use for the still-unfinished Redis Phase 0.5 item (see Progress checklist)
...
- [ ] CRUD on Redis — drafted but not finished/tested in Identity.Api (`Endpoints/CacheEndpoints.cs`, paused, still not created as of the 2026-09-09 survey); package refs added (`Aspire.Hosting.Redis` in apphost.cs, `Aspire.StackExchange.Redis` in Identity.Api), `cache` Redis resource registered and referenced by identity-api. **Now has an obvious real target**: caching Booking.Api's `/bookings/availability` results (keyed by `serviceId`+`date`, invalidated on booking create/cancel) — more useful than a throwaway Identity.Api proof; revisit here instead of `CacheEndpoints.cs` when picking this back up.
- [ ] Service Bus working across services — not started. Plan: Azure Service Bus via Aspire's local emulator (`Aspire.Hosting.Azure.ServiceBus` + `.RunAsEmulator()`, no real Azure account needed), Identity.Api publishes on `/register`, Notification.Api consumes and logs — confirmed packages/APIs via research, not yet implemented
...
- [ ] Validate `SalonId`/`ServiceId` on booking creation actually exist in Salon.Api (currently trusted, not checked)
- [ ] `POST /salons`'s trusted `OwnerId` (see Salon.Api section) — align with Booking.Api's JWT-derived-identity pattern
- [ ] Notification.Api: domain logic (currently template only) — natural fit once Service Bus (Phase 0.5) is done, so it can react to booking events
